...
export const user = pgTable(
  "Korisnik",
  {
    ...
    roleId: uuid("id_uloge").references(() => role.id, {
      onDelete: "set null",
    }),
  ...
  (table) => [
    check(
      "name_length",
      sql`length(${table.name}) >= 2 AND length(${table.name}) <= 50`,
    ),
    check(
      "email_format",
      sql`${table.email} ~* '^[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Z|a-z]{2,}$'`,
    ),
    check(
...
export const verification = pgTable("OauthVerifikacija", {
  ...
  createdAt: timestamp("datum_kreiranja").$defaultFn(
    () => /* @__PURE__ */ new Date(),
  ),
  updatedAt: timestamp("datum_azuriranja").$defaultFn(
    () => /* @__PURE__ */ new Date(),
  ),
})
...
export const recipe = pgTable(
  ...
  (table) => [
    ...
    check(
      "title_length",
      sql`length(${table.title}) >= 3 AND length(${table.title}) <= 100`,
    ),
...
export const recipeCategory = pgTable("Pripada_kategoriji", {
  recipeId: uuid("id_recepta")
    .notNull()
    .references(() => recipe.id, { onDelete: "cascade" }),
  categoryId: uuid("id_kategorije")
    .notNull()
...
export const recipeAllergy = pgTable("SadrziAlergiju", {
  recipeId: uuid("id_recepta")
    .notNull()
    .references(() => recipe.id, { onDelete: "cascade" }),
  allergyId: uuid("id_alergije")
    .notNull()
    .references(() => allergy.id, { onDelete: "cascade" }),
})
...
export const role = pgTable(
  ...
  (table) => [
    check(
      "name_length",
      sql`length(${table.name}) >= 2 AND length(${table.name}) <= 20`,
    ),
    check(
      "description_length",
      sql`${table.description} IS NULL OR length(${table.description}) >= 5`,
    ),
  ],
)
...
export const unit = pgTable(
  ...
  (table) => [
    ...
    check(
      "abbreviation_length",
      sql`length(${table.abbreviation}) >= 1 AND length(${table.abbreviation}) <= 10`,
    ),
    check(
      "type_length",
      sql`length(${table.type}) >= 2 AND length(${table.type}) <= 20`,
    ),
  ],
)
...
export const blog = pgTable(
  ...
  (table) => [
    check(
      "name_length",
      sql`length(${table.name}) >= 3 AND length(${table.name}) <= 100`,
    ),
    check(
      "description_length",
      sql`length(${table.description}) >= 10 AND length(${table.description}) <= 500`,
    ),
