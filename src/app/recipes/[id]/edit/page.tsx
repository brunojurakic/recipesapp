...
const EditRecipePage = ({ params }: EditRecipePageProps) => {
    ...
    const loadRecipe = async () => {
      try {
        ...
        const formData = {
          ...
          categories: recipeData.categories.map((cat: { category: { id: string } }) => cat.category.id),
          allergies: recipeData.allergies.map((allergy: { allergy: { id: string } }) => allergy.allergy.id),
          instructions: recipeData.instructions.map((inst: { stepNumber: number; content: string }) => ({
            stepNumber: inst.stepNumber,
            content: inst.content
          })),
          ingredients: recipeData.ingredients.map((ing: { name: string; quantity: number; unitId: string }) => ({
            name: ing.name,
  ...
  const onSubmit = async (data: CreateRecipeFormData) => {
    ...
    try {
      ...
      toast.error(error instanceof Error ? error.message : 'Greška pri ažuriranju recepta');
    } finally {
  ...
  const handleNext = async () => {
    ...
    const isStepValid = await trigger(fieldsToValidate as (keyof CreateRecipeFormData)[])
      ...
      <form className='space-y-6'>
        ...
        <div className='flex justify-between mt-8'>
          ...
          {currentStep < totalSteps ? (
            ...
            <Button type="button" onClick={handleUpdateRecipe} variant={'outline'} disabled={isSubmitting}>
              {isSubmitting ? (
                <>
