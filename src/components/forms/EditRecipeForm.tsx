...
const EditRecipeForm = ({ register, errors, image, setImage, imageError, existingImageUrl }: EditRecipeFormProps) => {
  ...
  return (
    <div className="space-y-6 mb-10">
      ...
      <div className="space-y-2">
        <Label htmlFor="image">Slika {existingImageUrl && !image && "(trenutna slika će se zadržati)"}</Label>
        ...
        {previewUrl && (
          <div className="mt-2">
            ...
            {!image && existingImageUrl && (
              <p className="text-sm text-muted-foreground mt-1">Trenutna slika recepta</p>
            )}
            {image && (
              <p className="text-sm text-muted-foreground mt-1">Nova slika - stara će biti zamijenjena</p>
            )}
          </div>
        )}
