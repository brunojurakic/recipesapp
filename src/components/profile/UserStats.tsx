...
export function UserStats({ className }: ClassNameProps) {
  ...
  const fetchUserStats = async () => {
    try {
      ...
      const totalReviews = recipes.reduce((sum: number, recipe: Recipe) => sum + recipe.reviews.length, 0);
      const totalRating = recipes.reduce((sum: number, recipe: Recipe) => {
        return sum + recipe.reviews.reduce((ratingSum: number, review: Review) => ratingSum + review.rating, 0);
      }, 0);
  ...
  if (!stats) {
    return (
      <Card className={className}>
        <CardContent className="flex items-center justify-center p-6">
          <p className="text-muted-foreground">Greška pri učitavanju statistike.</p>
        </CardContent>
      </Card>
    );
  }
        ...
        {stats.totalReviews > 0 && (
          <div className="mt-4 p-4 bg-muted rounded-lg">
            <div className="text-center">
              ...
              <div className="text-sm text-muted-foreground">
                Ukupno {stats.totalReviews === 1 ? 'recenzija' : 'recenzija'} na vašim receptima
              </div>
            </div>
          </div>
        )}
